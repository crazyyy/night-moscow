...
Description: Clean your Media Library.
Version: 3.2.8
Author: Jordy Meow
...
function wpmc_wp_ajax_wpmc_scan() {
	...
	if ( $medias ) {
		...
		echo json_encode(
			array( 'results' => $results, 'success' => true, 'message' => __( "Medias retrieved.", 'media-cleaner' ) )
		);
		die();
	}
		...
		if ( $isImage && isset( $meta['sizes'] ) ) {
			foreach ( $meta['sizes'] as $name => $attr ) {
				if  ( isset( $attr['file'] ) ) {
					...
					// create a new entry in the WP Cleaner DB to give the
...
function wpmc_check_file( $path ) {
	...
	if ( empty( $attachment_id_src ) || !file_exists( $source_filepath ) ) {
		...
		//this is an alternate thumbnail and the source is in the DB
...
function wpmc_admin_inline_js() {
	...
	echo 'var wpmc_cfg = { isPro: ' . ( wpmc_is_pro()  ? '1' : '0') . ', scanFiles: ' . ( ( wpmc_getoption( 'scan_files', 'wpmc_basics', false ) && wpmc_is_pro() ) ? '1' : '0' ) . ', scanMedia: ' . ( wpmc_getoption( 'scan_media', 'wpmc_basics', true ) ? '1' : '0' ) . ' };';
	echo "\n</script>";
}
...
function wpmc_screen() {
	$hide_ads = wpmc_getoption( 'hide_ads', 'wpmc_basics', false );
	?>
	<div class='wrap'>

		<?php $hide_ads ? "" : jordy_meow_donation(); ?>
		<h1>Media Cleaner <?php by_jordy_meow( $hide_ads ); ?></h1>
		...
		<div style='margin-top: 0px; background: #FFF; padding: 5px; border-radius: 4px; height: 28px; box-shadow: 0px 0px 6px #C2C2C2;'>
			...
			<!-- SCAN -->
			<?php if ( $view != 'deleted' ) { ?>
				<a id='wpmc_scan' onclick='wpmc_scan()' class='button-primary' style='float: left;'><img style='position: relative; top: 3px; left: -2px; margin-right: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( WP_PLUGIN_URL ) . trailingslashit( 'media-cleaner/img'); ?>search.png' /><?php _e("Scan", 'media-cleaner'); ?></a>
			<?php } ?>

			<!-- DELETE SELECTED -->
			<a id='wpmc_delete' onclick='wpmc_delete()' class='button' style='float: left; margin-left: 5px;'><img style='position: relative; top: 3px; left: -2px; margin-right: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( WP_PLUGIN_URL ) . trailingslashit( 'media-cleaner/img'); ?>delete.png' /><?php _e("Delete", 'media-cleaner'); ?></a>
			<?php if ( $view == 'deleted' ) { ?>
				<a id='wpmc_recover' onclick='wpmc_recover()' class='button-secondary' style='float: left; margin-left: 5px;'><img style='position: relative; top: 3px; left: -2px; margin-right: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( WP_PLUGIN_URL ) . trailingslashit( 'media-cleaner/img'); ?>pills.png' /><?php _e("Recover", 'media-cleaner'); ?></a>
			<?php } ?>

			<!-- IGNORE SELECTED -->
			<a id='wpmc_ignore' onclick='wpmc_ignore()' class='button' style='float: left; margin-left: 5px;'><img style='position: relative; top: 3px; left: -2px; margin-right: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( WP_PLUGIN_URL ) . trailingslashit( 'media-cleaner/img'); ?>tick.png' /><?php _e("Ignore", 'media-cleaner'); ?></a>

			<!-- RESET -->
			<?php if ( $view != 'deleted' ) { ?>
				<a id='wpmc_reset' href='?page=media-cleaner&reset=1' class='button-primary' style='float: right; margin-left: 5px;'><img style='position: relative; top: 3px; left: -2px; margin-right: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( WP_PLUGIN_URL ) . trailingslashit( 'media-cleaner/img'); ?>burn.png' /><?php _e("Reset", 'media-cleaner'); ?></a>
			<?php } ?>

			<!-- DELETE ALL -->
			<?php if ( $view == 'deleted' ) { ?>
				<a id='wpmc_recover_all' onclick='wpmc_recover_all()' class='button-primary' style='float: right; margin-left: 5px;'><img style='position: relative; top: 3px; left: -2px; margin-right: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( WP_PLUGIN_URL ) . trailingslashit( 'media-cleaner/img'); ?>pills.png' /><?php _e("Recover all", 'media-cleaner'); ?></a>
				<a id='wpmc_delete_all' onclick='wpmc_delete_all(true)' class='button button-red' style='float: right; margin-left: 5px;'><img style='position: relative; top: 3px; left: -2px; margin-right: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( WP_PLUGIN_URL ) . trailingslashit( 'media-cleaner/img'); ?>trash.png' /><?php _e("Empty trash", 'media-cleaner'); ?></a>
			<?php } else { ?>
				<a id='wpmc_delete_all' onclick='wpmc_delete_all()' class='button button-red' style='float: right; margin-left: 5px;'><img style='position: relative; top: 3px; left: -2px; margin-right: 3px; width: 16px; height: 16px;' src='<?php echo trailingslashit( WP_PLUGIN_URL ) . trailingslashit( 'media-cleaner/img'); ?>delete.png' /><?php _e("Delete all", 'media-cleaner'); ?></a>
			<?php } ?>
		...
		<p>
			<?php
				$scan_files = wpmc_getoption( 'scan_files', 'wpmc_basics', false ) && wpmc_is_pro();
				$scan_media = wpmc_getoption( 'scan_media', 'wpmc_basics', true );
				$hide_warning = wpmc_getoption( 'hide_warning', 'wpmc_basics', false );

				if ( !$hide_warning ) {
					_e( "<div class='error'><p><b style='color: red;'>Important!</b> <b>Backup your DB and your /uploads directory before using this plugin. </b> The delete files will be temporarily moved to the <b>uploads/wpmc-trash</b> directory. After testing your website, you can check the <a href='?page=media-cleaner&s&view=deleted'>trash</a> to either empty it or recover the files. This plugin is trying its best to stay safe. However, WordPress being a very dynamic system (plugins and themes are free to operate the way they want), it is impossible to predict all the situations in which your files are actually used. Again, <u>backup</u>.<br /><br /><b style='color: red;'>Another warning.</b> If you delete an item of the type <b>MEDIA</b>, its DB entry (related to the Media Library) will be <u>deleted permanently</u>. The files themselves will go in the trash. <br /><br /><b style='color: red;'>Please, be nice :)</b> Don't blame the plugin if it deleted (too many of) your files. This plugin makes cleaning possible, it is a difficult task only possible through this plugin, don't post a bad review because it broke your install. If you have a proper backup, there is actually <u>no risk</u>. Also, please help me <a target='_blank' href='https://wordpress.org/support/view/plugin-reviews/media-cleaner#postform'>by providing thoughtful reviews</a>, requesting features and sharing the bugs or potential enhancements you find with me. Thank you :)<br /><br /><b style='color: red;'>Sorry</b> for this long message, but I would rather be safe than sorry. You can disable this big warning in the option. Make sure you read it twice :)</p></div>", 'media-cleaner' );
				}

				if ( !$scan_files && !$scan_media ) {
					echo "<div class=\"error\"><p>";
					_e( "Scan is not enabled for neither the files or for the medias. The plugin is basically disabled. <b>Please check Settings > Media Cleaner</b>.", 'media-cleaner' );
					echo "</p></div>";
				}

				if ( !wpmc_is_pro() && !$hide_ads ) {
					echo "<div class=\"updated\"><p>";
					_e( "<b>This version is not Pro.</b> This plugin is a lot of work so please consider in getting the Pro version in order to receive support and to help the plugin to evolve. Also, the Pro version will also give you the option <b>to scan the <u>physical files</u> in your /uploads folder</b>. You can <a target='_blank' href='http://apps.meow.fr/media-cleaner'>get a serial for the Pro version here</a></b>.", 'media-cleaner' );
					echo "</p></div>";
				}

				echo sprintf( __( 'There are <b>%d issue(s)</b> with your files, accounting for <b>%d MB</b>. Your trash contains <b>%d MB.</b>', 'media-cleaner' ), $issues_count, number_format( $total_size / 1000000, 2 ), number_format( $trash_total_size / 1000000, 2 ) );
			?>
		</p>
...
function wpmc_validate_pro( $subscr_id ) {
	...
	$response = wp_remote_post( 'http://apps.meow.fr/wp-json/meow/v1/auth', array(
		'body' => array( 'subscr_id' => $subscr_id, 'item' => 'media-cleaner', 'url' => get_site_url() )
	) );
...
function wpmc_wp_enqueue_scripts () {
	...
	wp_enqueue_script( 'media-cleaner', plugins_url( '/media-cleaner.js', __FILE__ ), array( 'jquery' ), "3.0.4", true );
}
